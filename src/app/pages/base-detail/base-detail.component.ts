    ...
    dialogRef.afterClosed().subscribe((credentials) => {
      if (credentials) {
        ...
        this.airtable.submitMfa(credentials).subscribe({
          ...
          error: (err) => {
            const errorMessage = err.error?.message || 'Authentication failed. Check credentials.';
            this.snackBar.open(errorMessage, 'Close', { duration: 5000 });
            this.isSyncing.set(false);
            this.syncRevisionHistory(resumeCursor);
          },
        });
      }
    });
  ...
  private processScraperQueue(baseId: string, tableId: string, cursor?: string) {
    this.airtable.runScraper(baseId, tableId, cursor).subscribe({
      ...
      error: (err) => {
        ...
        if (
          err.error?.message === 'SCRAPER_AUTH_REQUIRED' ||
          err.status === 401 ||
          err.status === 403
        ) {
          this.snackBar.open(
            'Airtable session expired or invalid. Please re-authenticate.',
            'Close',
            { duration: 5000 },
          );
